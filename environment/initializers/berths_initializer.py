...
class BerthsInitializer:
    ...
    def __init__(
        ...
        world,
        filename,
        vessel_content_types,
        berth_service_distribution_factory,
        berth_randomized_check_prob=0,
    ):
        self.world = world
    ...
    def _create_berth(self, row):
        ...
        position = np.array([float(row["lon"]), float(row["lat"])])
        ...
        berth_info = BerthInfo(
            ...
            allowed_vessel_classes=self.berth_service_distribution_factory.get_allowed_vessel_classes_for_terminal(
                row["terminal"]
            ),
            section=row["section"],
        )

        sampler = self.berth_service_distribution_factory.service_time_sampler(
            row["terminal"]
        )
        ...
        self.world.add_component(
            berth, BerthStateMachine(sampler, self.berth_randomized_check_prob)
        )
