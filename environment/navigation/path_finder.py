...
class PathFinder:
    ...
    def load_traces(self,
        ...
        """
        assert ocean_spawn_ids is not None and len(ocean_spawn_ids) > 0, "No ocean spawn IDs provided!"
        self.ocean_spawn_ids = ocean_spawn_ids

        if ocean_tugs_folder is not None:
            rendezvous_dict, rendezvous = self._extract_traces_from_file(ocean_tugs_folder)
            self.tugs_rv, self.ocean_tugs_rv_traces = list(rendezvous), rendezvous_dict

        if ocean_pilots_folder is not None:
            rendezvous_dict, rendezvous = self._extract_traces_from_file(ocean_pilots_folder)
            self.pilots_rv, self.ocean_pilots_rv_traces = list(rendezvous), rendezvous_dict

        if pilots_rv_berth_traces_folder is not None:
            self.pilots_rv_berth_traces, _ = self._extract_traces_from_file(pilots_rv_berth_traces_folder)

        if tugs_rv_berth_traces_folder is not None:
            self.tugs_rv_berth_traces, _ = self._extract_traces_from_file(tugs_rv_berth_traces_folder)

        if pilots_rv_tugs_rv_traces_folder is not None:
            self.pilots_rv_tugs_rv_traces, _ = self._extract_traces_from_file(pilots_rv_tugs_rv_traces_folder)

        if tugs_wl_tugs_rv_traces_folder is not None:
            self.tugs_wl_tugs_rv_traces, _ = self._extract_traces_from_file(tugs_wl_tugs_rv_traces_folder)

        if pilots_wl_pilots_rv_traces_folder is not None:
            self.pilots_wl_pilot_rv_traces_dict, _ = self._extract_traces_from_file(pilots_wl_pilots_rv_traces_folder)

        if pilots_wl_berth_traces_folder is not None:
            self.pilots_wl_berth_traces_dict, _ = self._extract_traces_from_file(pilots_wl_berth_traces_folder)

        if ocean_tug_wl_traces_folder is not None:
            self.ocean_tug_wl_traces, _ = self._extract_traces_from_file(ocean_tug_wl_traces_folder)

        assert os.path.isdir(ocean_berth_folder), f"Ocean -> Berth Traces folder {ocean_berth_folder} does not exist"
        assert len(os.listdir(ocean_berth_folder)) > 0, f"No traces data in {ocean_berth_folder}"
    ...
    def load_tugs_rendezvous_locations(self, path):
        ...
        for feature in self._get_geojson_data(path)['features']:
            ...
            for final_destination in feature["properties"]["vessel_final_destination"]:
                tugs_rendezvous_mapping[str(final_destination)] = [point, feature["properties"]["id"]]
            ...
            for feature in self._get_geojson_data(path)['features']:
                ...
                if properties["usage"] == "arrival":
                    for vessel_class in properties["vessel_class"]:
                        pilots_arrival_rendezvous_mapping[vessel_class_map(vessel_class)].append(rendezvous_id)
                else:
                    for vessel_class in properties["vessel_class"]:
                        pilots_departure_rendezvous_mapping[vessel_class_map(vessel_class)].append(rendezvous_id)
    ...
    def ocean_tug_rendezvous_paths(self, vessel_position=None, final_section=None):
        ...
        """
        assert final_section is not None, "The final section of vessel path is required!"
    ...
    def ocean_pilot_rendezvous_paths(self, vessel_position=None, vessel_class=None):
        ...
        rendezvous_id = random.choice(self.pilots_arrival_rendezvous_mapping[vessel_class])
    ...
    def tug_rendezvous_berth_paths(self, vessel_position=None, final_section=None, berth_id=None):
        ...
        if len(paths) == 0:
            raise NoPathException(f"There does not exist a tug rendezvous {rendezvous_id} -> berth {berth_id} trace")
    ...
    def pilot_rendezvous_berth_paths(self, vessel_position=None, vessel_class=None, berth_id=None):
        ...
        if len(paths) == 0:
            raise NoPathException(f"There does not exist a pilot rendezvous -> {berth_id} trace")
    ...
    def pilot_rendezvous_tug_rendezvous_paths(self, pilot_rv_id, tug_rv_id):
        ...
        trace_ids = self._pilot_rendezvous_tug_rendezvous_trace_ids(pilot_rv_id, tug_rv_id)
        paths = []
    ...
    def prefix_path(self, point: list, path):
        ...
        path["point_sections"] = [self.sections_manager.ocean_section] + path["point_sections"]
    ...
    def tugs_ocean_waiting_location_path(self, tug_position: list, waiting_location_id):
        trace = random.choice(self.ocean_tug_wl_traces[f"ocean-tug_waiting_location:{waiting_location_id}"])
    ...
    def tugs_current_location_waiting_location_path(self, tug_position: list, waiting_location_id):
        paths = []
        ...
        try:
            ...
            raise NoPathException(f"There does not exist an ocean -> tug waiting location {waiting_location_id} trace")
    ...
    def tugs_waiting_location_rendezvous_path(self, tug_position: list, waiting_location_id, rendezvous_id):
        try:
            trace = random.choice(self.tugs_wl_tugs_rv_traces[f"tug_waiting_location:{waiting_location_id}-tug_rendezvous:{rendezvous_id}"])
        except KeyError:
            raise NoPathException(f"There does not exist a waiting location {waiting_location_id} -> rendezvous {rendezvous_id} trace")
    ...
    def tugs_berth_waiting_location_path(self, tug_position: list, berth_info, waiting_location_id):
        rendezvous_id = self.tug_rendezvous_id_for_berth(berth_info)

        rendezvous_wl_path = self.reverse_path(
            self.tugs_waiting_location_rendezvous_path(None, waiting_location_id, rendezvous_id))

        berth_rendezvous_paths, _ = self.tug_rendezvous_berth_paths(final_section=berth_info.section,
    ...
    def pilot_rendezvous_pilot_waiting_location_path(self, pilot_position: list, rendezvous_id, waiting_location_id):
        trace = random.choice(self.pilots_wl_pilot_rv_traces_dict[f"pilot_waiting_location:{waiting_location_id}-pilot_rendezvous:{rendezvous_id}"])
        trace = copy.deepcopy(trace)
    ...
    def pilot_waiting_location_pilot_rendezvous_path(self, pilot_position: list, rendezvous_id, waiting_location_id):
        trace = random.choice(
            self.pilots_wl_pilot_rv_traces_dict[f"pilot_waiting_location:{waiting_location_id}-pilot_rendezvous:{rendezvous_id}"])
        trace = copy.deepcopy(trace)
    ...
    def pilot_waiting_location_berth_path(self, pilot_position: list, berth_id, waiting_location_id):
        trace = random.choice(
            self.pilots_wl_berth_traces_dict[f"pilot_waiting_location:{waiting_location_id}-berth:{berth_id}"])
        trace = copy.deepcopy(trace)
    ...
    def berth_pilot_waiting_location_path(self, pilot_position: list, berth_id, waiting_location_id):
        trace = random.choice(
            self.pilots_wl_berth_traces_dict[f"pilot_waiting_location:{waiting_location_id}-berth:{berth_id}"])
        trace = copy.deepcopy(trace)
    ...
    def _ocean_berth_trace_ids(self, berth_id):
        return [
            f"ocean-berth:{berth_id}",
            f"berth:{berth_id}-ocean"
        ]
    ...
    def trim_trace_to_current_position(self, vessel_position, trace):
        ...
        for x, y in zip(xs, ys):
            distance = math.sqrt(math.pow(vessel_position[0] - x, 2) + math.pow(vessel_position[1] - y, 2))
