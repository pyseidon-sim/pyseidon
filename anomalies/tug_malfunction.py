...
from components import Position, VesselPath
...
from environment.queries import WaitingLocationList, berth_info_by_id
from exceptions import NoPathException, NotEnoughAvailableTugsException
...
class TugMalfunctionAnomaly(BaseVesselAnomaly):
    ...
    def execute_anomaly(self, entity_id):
        ...
        if vessel_id:
            ...
            if vessel_fsm.current() == VesselState.LEAVING:
                ...
                vessel_path = copy.deepcopy(
                    self.world.component_for_entity(vessel_id, VesselPath).path
                )
    ...
    def _replace_tug(self, vessel_id, tug_fsm, tug_id):
        ...
        self._assign_new_tug(
            vessel_id, vessel_fsm, tug_fsm.destination_berth_id, tug_rv_id, tug_pos
        )
    ...
    def _assign_new_tug(
        self, vessel_id, vessel_fsm, berth_id, old_tug_rendezvous_id, old_tug_pos
    ):
        ...
        try:
            ...
            self._schedule_assignment_timer(
                vessel_id, vessel_fsm, berth_id, old_tug_rendezvous_id, old_tug_pos
            )
            return
        ...
        for new_tug_entity_id in potential_tugs:
            try:
                new_tug_fsm = self.world.component_for_entity(
                    new_tug_entity_id, TugStateMachine
                )
                new_tug_position = self.world.component_for
                new_tug_path = self.world.component_for_entity(
                    new_tug_entity_id, VesselPath
                )
                ...
                if vessel_fsm.state_before_failure == VesselState.GOING_TO_BERTH:
                    ...
                    self.path_finder.trim_trace_to_current_position(
                        vessel_position=vessel_pos.lonlat, trace=tug_wl_destination_path
                    )

                    tug_wl_destination_path = self.path_finder.reverse_path(
                        tug_wl_destination_path
                    )
                    self.path_finder.prefix_path(
                        new_tug_position.lonlat, tug_wl_destination_path
                    )
                    ...
                    if self.deattach_location_polygon.contains(
                        Point(old_tug_pos.lon(), old_tug_pos.lat())
                    ):
                        ...
                        vessel_spawn_path = copy.deepcopy(
                            self.world.component_for_entity(vessel_id, VesselPath).path
                        )
                        vessel_spawn_path = self.path_finder.reverse_path(
                            vessel_spawn_path
                        )

                        self.path_finder.trim_trace_to_current_position(
                            vessel_position=vessel_pos.lonlat, trace=vessel_spawn_path
                        )
                        ...
                        tug_wl_destination_path = self.path_finder.merge_paths(
                            tug_wl_destination_path, vessel_spawn_path
                        )

                        new_tug_fsm.go_to_malfunction_location(vessel_id, berth_id)
                elif (
                    vessel_fsm.state_before_failure
    ...
    def _schedule_assignment_timer(
        self, vessel_id, vessel_fsm, berth_id, old_tug_rendezvous_id, old_tug_pos
    ):
    ...
    def _redirect_fixed_tug_to_waiting_location(self, tug_fsm, tug_path, tug_position):
        ...
        if tug_fsm.state_before_failure in [
            TugState.TUGGING_IN,
            TugState.GOING_TO_BERTH,
        ]:
            berth_info = berth_info_by_id(self.world, tug_fsm.destination_berth_id)

            tug_wl_destination_path, waiting_location_id = (
                self._find_berth_waiting_location_path(tug_position, berth_info)
            )

            if tug_wl_destination_path is None:
                raise Exception(
                    f"No path from berth {tug_fsm.destination_berth_id} to any tugboat waiting location"
                )
            else:
                tug_fsm.waiting_location_id = waiting_location_id
        elif tug_fsm.state_before_failure in [
            ...
            berth_info = berth_info_by_id(self.world, tug_fsm.destination_berth_id)
            berth_wl_path, waiting_location_id = self._find_berth_waiting_location_path(
                tug_position, berth_info
            )
            ...
            if berth_wl_path is None:
                ...
                tug_wl_destination_path = self.path_finder.merge_paths(
                    old_outgoing_path, berth_wl_path
                )
                tug_fsm.waiting_location_id = waiting_location_id
        else:
            raise Exception(
                f"Tug fsm was in an unhandled state: {tug_fsm.state_before_failure}"
            )

        tug_wl_destination_path = self.path_finder.trim_trace_to_current_position(
            vessel_position=tug_position.lonlat, trace=tug_wl_destination_path
        )
    ...
    def _find_berth_waiting_location_path(self, tug_position, berth_info):
        ...
        tug_waiting_locations = WaitingLocationList(
            world=self.world
        ).filter_by_location_type(LocationType.TUGBOATS_STORAGE)
        tug_waiting_locations = [
            location[1][0].id for location in list(tug_waiting_locations)
        ]
