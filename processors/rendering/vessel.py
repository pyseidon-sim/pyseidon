...
from components import Position, Course, VesselInfo, Velocity, VesselPath, BerthInfo
...
class VesselRenderer(BaseRenderer):
    ...
    def _process(self, dt):
        ...
        for vessel, (pos, course, vessel_info, fsm) in self.world.get_components(Position, Course, VesselInfo, VesselStateMachine):
            if not pos.is_valid():
            ...
            self._paint_on_mouse_hover(self.painter, x, y, self.mouse_x, self.mouse_y, vessel, vessel_info, pos, fsm)

    def _paint_on_mouse_hover(self, painter, x, y, mouse_x, mouse_y, ent, vessel_info, pos, fsm):
