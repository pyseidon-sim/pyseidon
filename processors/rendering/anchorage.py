...
class AnchorageRenderer(BaseRenderer):
    ...
    def _process(self, dt):
        for (_, [anchorage_info, anchorage_fsm, anchorage_shape]) in AnchorageList(world=self.world):
            self.paint_polygon(self.painter, anchorage_shape.polygon, self.ANCHORAGE_COLOR)

            center = np.array(anchorage_shape.centroid)
            x, y = self.proj.lonlat_to_screen(
                center[0],
                center[1])
