...
from components import FrameCounter, LocationType, Position, VesselPath
...
class DefaultTugStrategy:
        ...
        if frame_counter.get_count() >= self.RESOURCE_CHECK_FRAME_DELTA:
            ...
            if self.in_deattach_location(tug_position):
                ...
                _, (waiting_location_info, _) = (
                    WaitingLocationList(world=self.world)
                    .filter_by_location_type(LocationType.TUGBOATS_STORAGE)
                    .random_location()
                )
                waiting_location_path = (
                    self.path_finder.tugs_current_location_waiting_location_path(
                ...
                vessel_fsm = self.world.component_for_entity(
                    tug_fsm.current_target_vessel_id(), VesselStateMachine
                )
    ...
    def in_deattach_location(self, tug_position):
        return self.deattach_polygon.contains(
            Point(tug_position.lon(), tug_position.lat())
        )
    ...
    def _load_polygon(self, filename):
        ...
        return Polygon(area_json["features"][0]["geometry"]["coordinates"][0])
